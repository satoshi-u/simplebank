...
const getCountForUsers = `-- name: GetCountForUsers :one
SELECT COUNT(*) FROM users
`

func (q *Queries) GetCountForUsers(ctx context.Context) (int64, error) {
	row := q.db.QueryRowContext(ctx, getCountForUsers)
	var count int64
	err := row.Scan(&count)
	return count, err
}
